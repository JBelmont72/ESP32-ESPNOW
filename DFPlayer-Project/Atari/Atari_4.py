...
#improved version of Atari_3.py with smoother LFO implementation and float handling
def main():
    ...
    try:
        while True:
            ...
            # 1. Determine base frequency using the selector logic (remains mostly same)
            if val_mid > OFF_THRESHOLD:
                ...
                current_freq = 0.0 # Use float for silence/off

            # 2. Implement LFO for pitch modulation (vibrato effect)
            if val_lfo > OFF_THRESHOLD and current_freq > 0: # Check current_freq to avoid issues
                lfo_depth = map_value(val_lfo, OFF_THRESHOLD, 4095, 0.0, 10.0) # Use floats
                lfo_rate = map_value(val_lfo, OFF_THRESHOLD, 4095, 0.5, 5.0) # Use floats

                # The step size for the counter should ideally be linked to the time delta, 
                # but we can use a small fixed value for simplicity with time.sleep_ms(10)
                lfo_counter += 0.05 # Increment counter by a float
                if lfo_counter > 628:
                    lfo_counter = 0 # Reset counter (~2*pi*100)
            ...
            # Update the PWM output (no int() conversion here)
            if modulated_freq > 50 and volume_level > 0:
                pwm.freq(int(modulated_freq)) # Pass float frequency
                pwm.duty(volume_level) 
            else:
                pwm.duty(0)
        ...
        print("\nProgram terminated. PWM deinitialized.")
        sys.exit()
...
#         # Update the PWM output (no int() conversion here)
#         if modulated_freq > 50 and volume_level > 0:
#             pwm.freq(modulated_freq) # Pass float frequency
#             pwm.duty(volume_level) 
#         else:
#             pwm.duty(0)
